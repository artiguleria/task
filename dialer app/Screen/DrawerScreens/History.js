...
const History = ({navigation}) => {
  ...
  const getRequest = async () => {
    ...
    fetch(
      Config.API_URL + '/app/agent/report/' + userInfo.userid,
      {headers},
      {
        method: 'GET',
        //Request Type
  ...
  const ItemView = ({item}) => {
    return (
      // FlatList Item
      <View>
        <Text style={styles.text}>Customer : {item.customerNo}</Text>
        <Text style={styles.text}>Agent : {item.agentNo}</Text>
        <Text style={styles.text}>Call Time : {item.ansTime}</Text>
        <Text style={styles.text}>Call Duration : {item.leadTalkDuration}</Text>
    ...
    <SafeAreaView style={{flex: 1}}>
      <View style={styles.container}>
        <TextInput
          ...
          value={search}
          underlineColorAndroid="transparent"z
